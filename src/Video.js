...
class Video extends Component {
    ...
    try {
      ...
      await navigator.mediaDevices
        ...
        .then((value) => {
          ...
          //  let x =    audio_context.createMediaStreamSource(value);
          //   this.x = new MediaRecorder(value);

          // const processor = audio_context.createScriptProcessor(1024, 1, 1);
  ...
  getUserMedia = () => {
    if (
      ...
      console.log("trying to stop videos");
      try {
  ...
  getUserMediaSuccess = (stream) => {
    ...
    window.localStream = stream;
    console.log("local tracks is ", stream.getAudioTracks());
    this.localVideoref.current.srcObject = stream;

    for (let id in connections) {
      if (id === socketId) continue;

      console.log("adding track1 ", window.localStream.getAudioTracks());
      let data = { object: connections[id], tracks: [] };

      console.log("addded track1 ");
      window.localStream.getTracks().forEach((val) => {
        let a = connections[id].addTrack(val);
        data.tracks.push(a);
      });
      this.state.senders.push(data);
      console.log("adding stream1 ");
    ...
    socket.on("connect", () => {
      ...
      socket.on("user-joined", (id, clients) => {
        clients.forEach((socketListId) => {
          ...
          connections[socketListId].addEventListener("track", (e) => {
            // if(connections_streams[socketListId]==null || connections_streams[socketListId]==undefined){
            //   connections_streams[socketListId].

            // }
            // if(e.track.kind != 'video') return;
            //      console.log("track added ",e);

            // let a = new MediaStream();
            //a.addTrack(e.track);
            ...
            if (searchVidep !== null) {
              ...
              // searchVidep.srcObject =  a;

              console.log("some thing happed at this track", e.track);
              console.log("video object is  ", searchVidep.srcObject);
              let a = new MediaStream();
              if (e.track.kind == "video") {
                a.addTrack(searchVidep.srcObject.getAudioTracks()[0]);
                a.addTrack(e.track);
                searchVidep.srcObject = a;
              } else searchVidep.srcObject.addTrack(e.track);
            } else {
              elms = clients.length;
              let main = document.getElementById("main");
          ...
          if (window.localStream !== undefined && window.localStream !== null) {
            ...
            console.log("track is ", window.localStream.getTracks());
            let a = connections[socketListId].addTrack(
              window.localStream.getTracks()[1]
            );
            let b = connections[socketListId].addTrack(
              window.localStream.getTracks()[0]
            );
            //connections[socketListId].AudioContext
            let data = { object: connections[socketListId], tracks: [a, b] };
            this.state.senders.push(data);
            console.log("adding stream4 ");
          } else {
            let blackSilence = (...args) =>
              new MediaStream([this.black(...args), this.silence()]);
            window.localStream = blackSilence();
            connections[socketListId].addStream(window.localStream);
            console.log("adding stream5");
          }
        });
  ...
  handleVideo = () => {
    if (this.state.video) {
      ...
      tracks.forEach((track) => {
        console.log("stoping the track");

        track.stop();
      });
      console.log("sender is ", this.state.senders);
      this.state.senders.forEach((val) => {
        val.tracks.forEach((val2) => {
          console.log("removing track", val2);
          val.object.removeTrack(val2);
        });
      });

      // this.localVideoref.current.srcObject = null;
    } else {
      console.log("video is disbled");
    }
